      ...
      <div style={{ padding: "0 28px 28px" }}>
        ...
        {round.phase === "bidding" && (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.max(catPlayers.length, 1)},1fr)`, gap: 12, maxWidth: 1500, margin: "0 auto" }}>
              {catPlayers.map((p, i) => (
                <div key={p.id} className="popin" style={clayCardStyle({ overflow: "hidden" })}>
                  ...
                  <div style={{ padding: "14px 10px" }}>
                    ...
                    {p.duprRating != null && (
                      <div style={{ fontSize: 12, fontWeight: 800, color: C.tealDark, marginTop: 4 }}>DUPR {Number(p.duprRating).toFixed(3)}</div>
                    )}
                  </div>
                </div>
              ))}
          ...
          return (
            <div key={p.id} className="popin" style={clayCardStyle({ overflow: "hidden", opacity: won ? 1 : 0.75 })}>
              ...
              <div style={{ padding: "16px 14px" }}>
                ...
                {p.duprRating != null && <div style={{ fontSize: 12, color: C.inkSoft }}>DUPR {Number(p.duprRating).toFixed(3)}</div>}
                {won ? (
                  <>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
                {catPlayers.map((p) => (
                  ...
                  >
                    <span>{p.name}</span>
                    {p.duprRating != null && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.75 }}>DUPR {Number(p.duprRating).toFixed(3)}</span>}
                  </button>
                ))}
              </div>
      ...
      <div style={clayCardStyle({ padding: 18 })}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <ClaySelect value={assignPlayer} onChange={(e) => setAssignPlayer(e.target.value)} style={{ width: "auto", minWidth: 180 }}>
            ...
            {availablePlayers.map((p) => <option key={p.id} value={p.id}>{p.name} ({config.categoryNames[p.categoryId]}{p.duprRating != null ? `, DUPR ${Number(p.duprRating).toFixed(3)}` : ""})</option>)}
          </ClaySelect>
...
function SetupTab({ config, persistConfig, captains, persistCaptains, players, persistPlayers }) {
  ...
  const importPlayers = async () => {
    ...
    const newOnes = lines.map((line, i) => {
      ...
      const duprParsed = parts[2] !== undefined && parts[2] !== "" ? parseFloat(parts[2]) : null;
      const duprRating = duprParsed != null && !isNaN(duprParsed) ? duprParsed : null;
      const categoryId = CATEGORY_ORDER[catNum - 1];
      return categoryId ? { id: "p_" + Date.now().toString(36) + "_" + i, name, categoryId, duprRating, status: "available", winnerCaptainId: null, finalPrice: null } : null;
    }).filter(Boolean);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={clayCardStyle({ padding: 18 })}>
        ...
        <div style={{ fontSize: 12, color: C.inkSoft, marginBottom: 8 }}>One per line: <code>Player Name, Category Number (1-11), DUPR Rating (optional)</code></div>
        <textarea value={bulk} onChange={(e) => setBulk(e.target.value)} rows={6}
          ...
          placeholder={"Kasun Perera, 1, 4.250\nNimali Silva, 8, 3.875\n..."} />
