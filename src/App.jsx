...
const EMPTY_ROUND = {
  ...
  lastSale: null, // { playerId, captainId, price, categoryId, playerIndex, wasNoBid } - for undo
};
...
export default function App() {
  ...
  const pollAll = useCallback(async () => {
    ...
    setConfig(cfg); setCaptains(caps); setPlayers(plys); setRound(rnd); setLog(lg); setLoaded(true);
  }, []);
  ...
  const undoLastSale = async () => {
    ...
    await persistRound({
      ...round, categoryId: sale.categoryId, playerIndex: sale.playerIndex, phase: "bidding",
...
function PublicDisplay({ config, captains, players, round, teamName, onExit }) {
  ...
  const currentPlayer = catPlayers[round.playerIndex] || null;
  const now = useNow();
  const showingSold = round.phase === "sold-animation" && round.soldInfo && now < round.soldAnimationUntil;
            ...
            {showingSold ? (
              ...
              currentPlayer && (
                <div style={{ padding: "24px 0" }}>
                  ...
                  <div style={{ fontSize: 13, color: C.inkSoft, marginTop: 10 }}>Player {round.playerIndex + 1} of {catPlayers.length}</div>
                </div>
              )
            )}
          </>
...
function AuctioneerView({ config, captains, players, round, persistRound, persistPlayers, persistCaptains, persistLog, log, teamName, undoLastSale, onExit }) {
  ...
  useEffect(() => { setSelectedTeam(null); }, [round.categoryId, round.playerIndex]);
  ...
  const isLastPlayerInCategory = round.playerIndex >= catPlayers.length - 1;
  ...
  const skipNoBids = async () => {
    ...
    await persistRound({
      ...round, confirmedWinnerCaptainId: winner.id, categoryPlayersWonIds: [...wonTeamIds, winner.id],
      ...
      lastSale: { playerId: currentPlayer.id, captainId: winner.id, price, categoryId: catId, playerIndex: round.playerIndex, wasNoBid: true },
    });
  };
  const nextPlayerOrCategory = async () => {
    if (!isLastPlayerInCategory) {
      await persistRound({ ...round, playerIndex: round.playerIndex + 1, phase: "bidding", timerEndsAt: Date.now() + BID_SECONDS * 1000, confirmedWinnerCaptainId: null, soldAnimationUntil: null, soldInfo: null });
    } else {
      const nextCat = findNextCategoryId(config, players);
      if (nextCat) {
        await persistRound({ ...round, categoryId: nextCat, playerIndex: 0, phase: "bidding", timerEndsAt: Date.now() + BID_SECONDS * 1000, confirmedWinnerCaptainId: null, categoryPlayersWonIds: [], soldAnimationUntil: null, soldInfo: null });
      } else {
    ...
    <div style={{ minHeight: "100vh", background: C.bg }}>
      ...
      <div style={{ padding: "0 16px 24px", maxWidth: 480, margin: "0 auto" }}>
        ...
        {currentPlayer && round.phase === "bidding" && (
          <div>
            <Chip bg={C.pinkSoft} fg={C.pinkDark}>{catName} — Player {round.playerIndex + 1}/{catPlayers.length}</Chip>
            ...
            <div style={{ textAlign: "center", marginTop: 10 }}>
              <TextLink onClick={skipNoBids}>No bids on this player — skip &amp; randomly assign</TextLink>
            </div>
          </div>
        )}
        ...
        {round.phase === "sold-animation" && round.soldInfo && (
          <div style={{ textAlign: "center", padding: "30px 0" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: C.ink, marginTop: 4 }}>{teamName(round.soldInfo.captainId)} — {money(round.soldInfo.price)}</div>
            <div style={{ marginTop: 18 }}>
              <ClayButton color={C.lime} darkShadow={C.limeDark} disabled={!animationDone} onClick={nextPlayerOrCategory}>
                {isLastPlayerInCategory ? "Next Category" : "Next Player"}
              </ClayButton>
            </div>
          </div>
        )}
...
function LiveAuctionTab({ config, captains, players, round, persistRound, persistPlayers, persistCaptains, log, persistLog, teamName, undoLastSale }) {
  ...
  useEffect(() => { setPriceInput(catId ? String(config.categoryReserves[catId] ?? "") : ""); }, [round.playerIndex, round.categoryId, round.phase]); // eslint-disable-line

  const pushCategory = async (categoryId) => {
    if (!categoryId) return;
    await persistRound({ ...round, categoryId, playerIndex: 0, phase: "bidding", timerEndsAt: Date.now() + BID_SECONDS * 1000, confirmedWinnerCaptainId: null, categoryPlayersWonIds: [], soldAnimationUntil: null, soldInfo: null });
  };
  ...
  const confirmPrice = async () => {
    ...
    await persistRound({
      ...round, categoryPlayersWonIds: [...wonTeamIds, capId], phase: "sold-animation", soldAnimationUntil: Date.now() + SOLD_ANIMATION_MS,
      soldInfo: { playerId: currentPlayer.id, captainId: capId, price, wasNoBid: false },
      lastSale: { playerId: currentPlayer.id, captainId: capId, price, categoryId: catId, playerIndex: round.playerIndex, wasNoBid: false },
    });
  };
          ...
          <>
            ...
            {currentPlayer && (
              <div style={{ fontSize: 15, color: C.inkSoft, marginBottom: 10 }}>
                Player {round.playerIndex + 1}/{catPlayers.length}: <b style={{ color: C.ink }}>{currentPlayer.name}</b>
